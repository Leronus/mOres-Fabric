...
public class ModPickaxeItem extends PickaxeItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if(stack.getItem() == ModItems.AMETHYST_PICKAXE) {
            target.addStatusEffect(new StatusEffectInstance(StatusEffects.LEVITATION, 250, 1, false, false));
        }
        //If the item is an onyx pickaxe, apply wither effect on targetEntity
        ...
        //If the item is a ruby pickaxe, apply fire effect on targetEntity
        if(stack.getItem() == ModItems.RUBY_PICKAXE) {
            target.setOnFireFor(5);
        }
        //If the item is a sapphire sword, apply glowing effect on targetEntity
        if(stack.getItem() == ModItems.SAPPHIRE_PICKAXE || stack.getItem() == ModItems.SAPPHIRE_DAGGER) {
            target.addStatusEffect(new StatusEffectInstance(StatusEffects.GLOWING, 250, 1, false, false));
        }
        //If the item is a moissanite pickaxe, apply poison effect on targetEntity
        if(stack.getItem() == ModItems.MOISSANITE_PICKAXE) {
        ...
        if (!attacker.getWorld().isClient
                && (stack.getItem() == ModItems.SPINEL_PICKAXE)
